...
def get_labels(data, fill=["number"]):
    ...
    # Jung Woo Lee Nov. 20, 2023
    # Print the elements in the sets onto the venn diagram sections
    if "types" in fill:
        for k in set_collections:
            type_list = list(set_collections[k])
            data_string =  ""
            if len(type_list) > 0:
              data_string = "<"
              for i, type in enumerate(type_list):
                  data_string += type
                  if i < len(type_list) - 1:  # Check if it's not the last element
                      data_string += "\n"
              data_string += '>'
            labels[k] += data_string
